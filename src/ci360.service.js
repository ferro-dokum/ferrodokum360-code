...
class CI360Viewer {
  ...
  update() {
    ...
    if (this.fullScreenView) {
      ...
      this.canvas.height =
        ((this.container.offsetWidth * this.devicePixelRatio) / image.width) *
        image.height *
        0.5
      this.canvas.style.height =
        (this.container.offsetWidth / image.width) * image.height * 0.5 + 'px'
  ...
  onFirstImageLoaded(event) {
    ...
    if (this.fullScreenView) {
      ...
      this.canvas.height =
        ...
        event.target.height
      this.canvas.style.height =
        (this.container.offsetWidth / event.target.width) *
          event.target.height +
        'px'
  ...
  getSrc(
    ...
    folder,
    filename,
    { ciSize, ciToken, ciOperation, ciFilters }
  ) {
    ...
    if (responsive) {
      ...
      const ciSizeNext = getSizeAccordingToPixelRatio(
        ciSize || getResponsiveWidthOfContainer(imageOffsetWidth)
      )
  ...
  initControls() {
    ...
    if (prev) {
      ...
      prev.addEventListener(
        'touchstart',
        isReverse ? onRightStart : onLeftStart
      )
    ...
    if (next) {
      ...
      next.addEventListener(
        'touchstart',
        isReverse ? onLeftStart : onRightStart
      )
